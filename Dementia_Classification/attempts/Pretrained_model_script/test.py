...
def main():
    ...
    class CombinedModel(nn.Module):
        def __init__(self, vgg16, tuned_cnn, custom_head):
            super(CombinedModel, self).__init__()
            self.vgg16_features = vgg16
            self.tuned_cnn = tuned_cnn
            self.custom_head = custom_head

        def forward(self, x):
            x_vgg16 = x.repeat(1, 3, 1, 1)
            vgg16_output = self.vgg16_features(x_vgg16)
            vgg16_output = vgg16_output.view(vgg16_output.size(0), -1)

            tuned_cnn_output = self.tuned_cnn(x)
            tuned_cnn_output = tuned_cnn_output.view(tuned_cnn_output.size(0), -1)

            combined_output = torch.cat((vgg16_output, tuned_cnn_output), dim=1)
            final_output = self.custom_head(combined_output)
            return final_output
    ...
    tuned_cnn = TunedCNN()
    custom_head = CustomHead(input_dim=25088 + 4, num_classes=4)

    combined_model = CombinedModel(vgg16_features, tuned_cnn, custom_head).to(device)

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.AdamW(combined_model.parameters(), lr=0.0001)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=7, gamma=0.1)

    def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=100, patience=3):
        best_loss = float('inf')
        patience_counter = 0
    ...
    def train_and_evaluate(dataloaders):
        ...
        model = train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=100, patience=3)
