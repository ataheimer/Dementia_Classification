...
def main():
    data_dir = '../Output_filtered/processed'  # Path to augmented images
    ...
    class MobileNetV2Model(nn.Module):
        def __init__(self, num_classes):
            ...
            self.model.avgpool = nn.AdaptiveAvgPool2d((1, 1))  # Add AdaptiveAvgPool2d layer

        def forward(self, x):
            x = self.model.features(x)
            x = self.model.avgpool(x)  # Apply AdaptiveAvgPool2d
            x = torch.flatten(x, 1)
